...
right = 642
right_total = 1612
precision_total = 1024.4805555555552
recall_total = 1164.6011904761901
f_score_total = 1061.2928626928588
...
def call_api_with_retry(masked_method, base_url, max_retries=5, initial_delay=1):
    """
    手动创建JSON请求调用API并带有重试机制
    """
    headers = {
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": f"方法体：{masked_method}\n<mark>是已被覆盖的方法名，根据上面的方法体进行<mark>位置的方法名预测，"
                                        f"最后给出一个驼峰式方法名即可。（注意：代码方法名是一个词，只由字母、数字或_组成，不要给出多余的中文、字符、引号。）"}
        ]
    }
        ...
        if isHaveNote(method) is None:
            ...
            if total > right_total * 400:
                if total % 400 == 0:
                    ...
                    recommend_name = call_api_with_retry(masked_method, base_url)
                    index = index + 1
